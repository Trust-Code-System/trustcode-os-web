...
- Accounts seeded or created before the invitation flow have a null `emailVerifiedAt` and cannot sign in until they verify. Verification is also required by the JWT strategy and refresh, so an unverified user is rejected on every protected route.
