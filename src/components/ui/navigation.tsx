...
export function PageHeader({ eyebrow, title, description, actions, breadcrumb }: { eyebrow?: string; title: string; description?: string; actions?: ReactNode; breadcrumb?: ReactNode }) { return <header className="mb-4 grid gap-2"><div>{breadcrumb}{eyebrow ? <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-text-muted">{eyebrow}</p> : null}<div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h1 className="text-pretty break-words text-[1.65rem] font-bold leading-tight tracking-[-0.04em]">{title}</h1>{description ? <p className="mt-1 max-w-3xl text-xs text-text-secondary">{description}</p> : null}</div>{actions ? <div className="flex max-w-full min-w-0 flex-wrap gap-2">{actions}</div> : null}</div></div></header>; }

export function SectionHeader({ title, description, action }: { title: string; description?: string; action?: ReactNode }) { return <div className="mb-3 flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-sm font-bold tracking-[-0.015em]">{title}</h2>{description ? <p className="mt-1 text-xs text-text-secondary">{description}</p> : null}</div>{action}</div>; }
