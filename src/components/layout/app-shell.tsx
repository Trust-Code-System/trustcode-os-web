      ...
      <div className="app-frame">
        <aside className="hidden h-full w-[4.25rem] shrink-0 flex-col items-center bg-sidebar lg:flex">
          <Link
            ...
            className="mt-3 block size-10 overflow-hidden rounded-[0.85rem] shadow-[0_8px_18px_rgb(27_63_174/0.24)]"
          >
            <BrandMark />
          </Link>
          <nav
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          <header className="z-20 flex h-16 shrink-0 items-center gap-2 border-b border-border/70 bg-surface px-3 md:px-4 lg:pl-0">
            ...
            <button
              ...
              className="ml-auto flex min-h-9 min-w-0 items-center gap-2 rounded-full border bg-surface px-3 text-left text-xs text-text-muted shadow-[var(--shadow-inset)] hover:border-border-strong hover:bg-surface-hover sm:w-48 lg:ml-auto"
            >
              <Search aria-hidden className="size-4 shrink-0" />
              <span className="hidden min-w-0 flex-1 truncate sm:inline">
                Search workspace
              </span>
              <kbd className="ml-auto hidden rounded-md border px-1.5 py-0.5 font-mono text-[9px] xl:inline">
                ⌘K
              </kbd>
            </button>
            <Popover
            ...
            <DropdownMenu
              ...
              trigger={
                <button
                  type="button"
                  className="ml-1 rounded-full ring-offset-2 focus-visible:outline-2 focus-visible:outline-focus lg:hidden"
                >
                  <Avatar name={user.name ?? user.email} {...(user.avatarUrl ? { src: user.avatarUrl } : {})} size="sm" />
                </button>
              }
          ...
          <main
            id="main-content"
            className="min-h-0 flex-1 overflow-y-auto overscroll-contain bg-workspace px-3 py-4 md:px-5 md:py-5"
          >
            {children}
          </main>
        </div>
          ...
          return (
            <li key={item.href}>
              <Link
                ...
                aria-current={active ? "page" : undefined}
                className={cn(
                  "flex min-h-11 items-center gap-3 rounded-[var(--radius-md)] px-3 text-sm font-semibold text-text-secondary hover:bg-surface-hover hover:text-text-primary",
                  active && "bg-surface-active text-brand",
                )}
              >
