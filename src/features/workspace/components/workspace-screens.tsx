...
import {
  ...
  type MeetingInput, type ProjectInput, type ProjectRole, type ProjectUpdateInput,
} from "../api/workspace";
...
export function ProjectsScreen() {
  ...
  const create = useMutation({ mutationFn: createProject, onSuccess: async () => { setAdding(false); await refreshAppData(queryClient); }, meta: { successMessage: "Project created" } });
...
export function ProjectWorkspaceScreen({ projectId }: { projectId: string }) {
  ...
  const status = useMutation({ mutationFn: (value: ProjectStatus) => changeProjectStatus(projectId, value), onSuccess: refresh, meta: { successMessage: "Project status updated" } });
  const priorityUpdate = useMutation({ mutationFn: (value: ProjectPriority) => updateProject(projectId, { priority: value }), onSuccess: refresh, meta: { successMessage: "Project priority updated" } });
  const archive = useMutation({ mutationFn: (value: boolean) => setProjectArchived(projectId, value), onSuccess: refresh, meta: { successMessage: "Project archive status updated" } });
  const milestoneCreate = useMutation({ mutationFn: (input: { title: string; description?: string; dueDate?: string }) => createMilestone(projectId, input), onSuccess: refresh, meta: { successMessage: "Milestone created" } });
  const milestoneComplete = useMutation({ mutationFn: (id: string) => completeMilestone(projectId, id), onSuccess: refresh, meta: { successMessage: "Milestone completed" } });
  const milestoneDelete = useMutation({ mutationFn: (id: string) => deleteMilestone(projectId, id), onSuccess: refresh, meta: { successMessage: "Milestone deleted" } });
  const milestoneUpdate = useMutation({ mutationFn: (input: { id: string; status: "PENDING" | "IN_PROGRESS" | "COMPLETED" }) => updateMilestone(projectId, input.id, { status: input.status }), onSuccess: refresh, meta: { successMessage: "Milestone updated" } });
  const memberAdd = useMutation({ mutationFn: (input: { userId: string; role: (typeof projectRoles)[number] }) => addProjectMember(projectId, input.userId, input.role), onSuccess: refresh, meta: { successMessage: "Project member added" } });
  const memberRemove = useMutation({ mutationFn: (userId: string) => removeProjectMember(projectId, userId), onSuccess: refresh, meta: { successMessage: "Project member removed" } });
  const memberRole = useMutation({ mutationFn: (input: { userId: string; role: ProjectRole }) => updateProjectMemberRole(projectId, input.userId, input.role), onSuccess: refresh, meta: { successMessage: "Project member role updated" } });
...
export function ProjectEditScreen({ projectId }: { projectId: string }) {
  ...
  const update = useMutation({ mutationFn: (input: ProjectUpdateInput) => updateProject(projectId, input), onSuccess: () => refreshAppData(queryClient), meta: { successMessage: "Project updated" } });
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    update.mutate({
      ...
      priority: String(data.get("priority") ?? project.priority) as ProjectPriority,
      ...(startDate ? { startDate: new Date(startDate).toISOString() } : {}),
      ...(dueDate ? { dueDate: new Date(dueDate).toISOString() } : {}),
    });
  };

  return <><PageHeader title="Edit project" description={project.name} actions={<Link href={`/projects/${project.id}`} className={buttonVariants({ variant: "secondary" })}>Back to project</Link>} />{project.archivedAt ? <Alert variant="warning" title="Project is archived">Restore this project before editing it.</Alert> : null}<Panel title="Project details"><form onSubmit={submit} className="grid gap-3 sm:grid-cols-2"><FormField label="Project name" required><Input name="name" required minLength={2} defaultValue={project.name} disabled={Boolean(project.archivedAt)} /></FormField><FormField label="Priority"><Select name="priority" defaultValue={project.priority} disabled={Boolean(project.archivedAt)}>{projectPriorities.map((value) => <option key={value} value={value}>{label(value)}</option>)}</Select></FormField><FormField label="Start date"><Input type="date" name="startDate" defaultValue={project.startDate ? format(new Date(project.startDate), "yyyy-MM-dd") : ""} disabled={Boolean(project.archivedAt)} /></FormField><FormField label="Due date"><Input type="date" name="dueDate" defaultValue={project.dueDate ? format(new Date(project.dueDate), "yyyy-MM-dd") : ""} disabled={Boolean(project.archivedAt)} /></FormField><div className="sm:col-span-2"><FormField label="Description"><Textarea name="description" defaultValue={project.description ?? ""} disabled={Boolean(project.archivedAt)} /></FormField></div><div className="flex items-center gap-3 sm:col-span-2"><Button type="submit" loading={update.isPending} disabled={Boolean(project.archivedAt)}>Save changes</Button>{update.isSuccess ? <span className="text-sm text-success">Project updated.</span> : null}</div>{update.error ? <div className="sm:col-span-2"><Alert variant="danger" title="Project could not be updated">{toAppError(update.error).message}</Alert></div> : null}</form></Panel></>;
}
...
export function MeetingsScreen() {
  ...
  const create = useMutation({ mutationFn: createMeeting, onSuccess: async () => { setAdding(false); await refreshAppData(queryClient); }, meta: { successMessage: "Meeting scheduled" } });
  const cancel = useMutation({ mutationFn: cancelMeeting, onSuccess: () => refreshAppData(queryClient), meta: { successMessage: "Meeting cancelled" } });
  const reschedule = useMutation({ mutationFn: ({ id, startsAt }: { id: string; startsAt: string }) => updateMeeting(id, { startsAt }), onSuccess: () => refreshAppData(queryClient), meta: { successMessage: "Meeting rescheduled" } });
...
export function TeamScreen() {
  ...
  const invite = useMutation({ mutationFn: inviteUser, onSuccess: async () => { setInviting(false); await refreshAppData(queryClient); }, meta: { successMessage: "Invitation sent" } });
  const active = useMutation({ mutationFn: ({ id, value }: { id: string; value: boolean }) => setUserActive(id, value), onSuccess: () => refreshAppData(queryClient), meta: { successMessage: "Team member access updated" } });
  const roleUpdate = useMutation({ mutationFn: ({ id, role }: { id: string; role: "ADMIN" | "MEMBER" }) => updateUser(id, { role }), onSuccess: () => refreshAppData(queryClient), meta: { successMessage: "Team member role updated" } });
...
function InvitationsPanel() {
  ...
  const resend = useMutation({ mutationFn: resendInvitation, onSuccess: invalidate, meta: { successMessage: "Invitation resent" } });
  const revoke = useMutation({ mutationFn: revokeInvitation, onSuccess: invalidate, meta: { successMessage: "Invitation revoked" } });
