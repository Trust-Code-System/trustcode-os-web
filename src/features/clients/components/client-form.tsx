...
function ClientFormFields({ clientId, existing }: { clientId?: string; existing?: Client }) {
  ...
  const save = useMutation({ mutationFn: (input: ClientInput) => clientId ? updateClient(clientId, input) : createClient(input), onSuccess: (client) => router.push(`/clients/${client.id}`) });
