...
function ClientFormFields({ clientId, existing }: { clientId?: string; existing?: Client }) {
  ...
  const save = useMutation({ mutationFn: (input: ClientInput) => clientId ? updateClient(clientId, input) : createClient(input), onSuccess: async (client) => { await refreshAppData(queryClient); router.push(`/clients/${client.id}`); }, meta: { successMessage: clientId ? "Client updated" : "Client created" } });
