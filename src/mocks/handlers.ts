...
const activities = clients.flatMap((client) => client.activity);
const users = [
...
export const handlers = [
  ...
  http.get("*/api/backend/activity", ({ request }) => { const url = new URL(request.url); const clientId = url.searchParams.get("clientId"); const projectId = url.searchParams.get("projectId"); const actorId = url.searchParams.get("actorId"); const entityType = url.searchParams.get("entityType"); const action = url.searchParams.get("action"); const page = Math.max(1, Number(url.searchParams.get("page") ?? 1)); const pageSize = Math.min(100, Math.max(1, Number(url.searchParams.get("pageSize") ?? 20))); const rows = activities.filter((item) => (!clientId || item.clientId === clientId) && (!projectId || item.projectId === projectId) && (!actorId || item.actorId === actorId) && (!entityType || item.entityType === entityType) && (!action || item.action === action)); return success(rows.slice((page - 1) * pageSize, page * pageSize), { page, pageSize, total: rows.length }); }),
